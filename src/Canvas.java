...
class Idx {
    ...
    public Idx(int sx, int sy, Pixel col) {
        this.width = Math.max(sx, 1);
        this.height = Math.max(sy, 1);
        this.data = new int[this.width * this.height * 4];
        int i = 0;
        while (i < this.data.length) {
            this.data[i++] = col.r;
    ...
    public Pixel getChoordPixel(double x, double y) {
        ...
        // int m = 4*( ( (int)(this.width*x) )%this.width +
        // (((int)(this.height*y))%this.height)*this.width);
    ...
    void setPixelAtCoord(double u, double v, Pixel p) {
        int x, y;
        x = (int) (u * this.width) % this.width;
        y = (int) (v * this.height) % this.height;
        int m = this.mx(x, y);
        this.data[m++] = p.r;
        this.data[m++] = p.g;
        this.data[m++] = p.b;
    ...
    public Idx getResizedIdx(int sx, int sy) {
        if (sx == this.width && sy == this.height)
            return this;
        Idx n = new Idx(sx, sy);
        int ax, ay;
        for (int y = 0; y < sy; y++) {
            ay = (int) (((double) y / sy) * this.height);
            for (int x = 0; x < sx; x++) {
                ax = (int) (((double) x / sx) * this.width);
                n.setPixel(x, y, this.getPixel(ax, ay));
            }
        }
        return n;
    }
